...
|---|------|-------------|--------|
| 1 | Landing -> Lab | CTA click, initial data load, screener populates with real player data | RE-AUDIT SESSION 10 — PASS (functional). 0 JS errors, 25 rows load. But FUNC-012 L2: Barkley 2504 rush yds (actual 2005, +25%), Lamar 19 GP (actual 17). Data works, numbers lie. |
...
| 15 | Player profile: Game log | Individual game stats shown? Sum of game log = season total? Week numbers correct? | RE-AUDIT SESSION 10 — FUNC-012 L2. Lamar 2024 game log: 19 weeks shown (17 reg + weeks 19-20 playoff). Playoff weeks appear as regular weeks with NO visual distinction. User cannot tell which stats are regular season. |
...
| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 18 | Dynasty Rankings | Rankings load? Sortable? Position filter? Do rankings reflect age + production reality? | RE-AUDIT SESSION 10 — FUNC-012 L2. Nacua=19 GP (should be 17). dynasty.py has 0 season_type filters. Rankings functionally work but underlying game counts are inflated for all playoff players, skewing production component of dynasty values. |
...
| 27 | Matchups | Loads? Correct matchups for selected week? Opponent data shown? | RE-AUDIT SESSION 10 — STILL BROKEN. Matchup heatmap defense grid correct (PHI=17, ARI=17, ATL=17). BUT weekly heatmap still returns weeks 1-22. FUNC-012 Layer 2 untouched by Ship Loop (Phases B-E were visual/UX only). |
...
| 62 | Dashboard / Stat Leaders | Summary stats populated? Leaders match screener data? Category switching works? | RE-AUDIT SESSION 10 — STILL BROKEN. Stat Leaders: Lamar=19 GP (PPG=24.8), Allen=19 GP. Dynasty Dashboard top5: Nacua=19 GP. Endpoints WITH filters correct (Awards=17, Reports=17, SOS correct). FUNC-012 Layer 2 untouched. |
