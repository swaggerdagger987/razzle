...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 54 — PASS locally. Prod: IEEE 754 artifacts persist (FUNC-059, 13/20 top players). GET sort bug (FUNC-042) still on prod. snap_share filter excludes players with null snap data — prod has correct snap data, local DB missing (FUNC-058). |
| 37 | Regression candidates | TD regression logic sound? Flagging high-TD players with low expected TDs? | RE-AUDIT SESSION 53 — FUNC-055 FIXED (Ship Loop 4ed32ce). regression.html now uses correct API fields (positive_regression, negative_regression, pos_avg_td_rates, td_diff). API verified: 50 buy-low + 50 sell-high candidates, td_diff signs correct (positive=neg_regression, negative=pos_regression). FUNC-056 FIXED: pace panel uses /api/pace-tracker which returns milestone objects {label, target, current, projected}. Ship Loop fix (p.projections→p.milestones, s.milestone→s.target) matches API. McCaffrey 1000 rush yd ON PACE verified. Both panels Pro-gated. |
