...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 28 — STILL FAILING. FUNC-021/022 confirmed still live on prod. Prescott PPO=5.92 (all top 5 most_efficient are QBs). Stafford td_rate=158.6%. Ship Loop did not address these. Session 28 added: full RB/WR cross-verification PASS (McCaffrey td_rate=3.9%, catch_rate=79.1% exact match to manual calc). Field name clarification: pass attempts = `attempts` (not `passing_attempts`), completion % = `comp_pct`. FUNC-022 ticket updated with precise fix code using existing `att` variable from line 241. |
