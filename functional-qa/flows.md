...
|---|------|-------------|--------|
| 1 | Landing -> Lab | CTA click, initial data load, screener populates with real player data | RE-AUDIT SESSION 26 — PASS. Prod: 0 JS errors, 25 rows, 607 players (2025 season), 69 sidebar items. Landing page: 0 JS errors. Player profile and compare pages load cleanly. All Ship Loop session 24-25 fixes verified in code. |
...
| 7 | Screener: Week filter | Select Week 1. Stats are single-week, not season totals? Switch back to All Weeks. | RE-AUDIT SESSION 27 — PARTIAL FAIL. Core stats (passing_yards, GP) correctly week-scoped. P1: PBP stats (scramble_attempts, gl_carries, garbage_time_pct, etc.) always return season totals regardless of week filter (FUNC-023). |
...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 26 — FAIL. Two P0 tickets open: FUNC-021 (PPO broken for QBs — all top 10 "most efficient" are QBs on prod, Prescott PPO=5.92 vs McCaffrey 0.95) and FUNC-022 (screener td_rate/fumble_rate broken for QBs — Stafford td_rate=158.6% on prod because 46 TDs / 29 rushing carries). Root cause: position-agnostic formulas that use touchdowns/(carries+targets) — for QBs, passing TDs in numerator but pass attempts not in denominator. Fix exists in analytics.py:909 (correct QB-specific formula) but not applied in core.py, dashboards.py, players.py. Non-QB efficiency stats (YPC/YPR/YPT/CR/CompPct) all verified correct. |
