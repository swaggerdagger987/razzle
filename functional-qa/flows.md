...
| 35 | Opportunity Share | Opportunity = targets + carries? Per-team shares sum to ~100%? | RE-AUDIT SESSION 35 — PASS. FUNC-005 RESOLVED on prod: WR dominator data now working (Adams dom=46.9, JSN dom=42.1, Olave dom=38.1). Alpha dogs correct (McCaffrey opp%=44.0, Jeanty 39.4, Bijan 39.2). |
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 35 — STILL BROKEN ON PROD. Code fix verified (dashboards.py:78,121-122). Prod: Dak opp=53 (carries only), should be 653 (600 att + 53 carries). FUNC-035 filed. Deployment gap (FUNC-036). |
...
| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 63 | Bureau: League Intel | Sleeper connect flow? Roster loads? Insights generated from real league data? | RE-AUDIT SESSION 34 — PASS. Ship Loop Phase C (Bureau hardening) verified. Demo league: 10 managers, championship odds, warning banner, styled correctly. Connect card centered, dark mode clean. Bureau APIs: Monte Carlo returns distributions (Lamar mean=25.32, stdev=6.19), roster-depth returns PPG, trade-finder correctly auth-gated. 0 JS errors. AbortController timeouts on all API calls. C-1 to C-7 all verified. |
...
| 72 | Agent Presence Layer | Peek fires? Watermark renders? Nudges show for Elite? Config correct? | DONE SESSION 36 — PASS with FUNC-039. agent-config.js: 6 agents, column/panel lookups, null guards. Peek: 1/7 random, dismiss on click/Enter, opt-out via localStorage. Watermark: preloaded SVGs, img guard, globalAlpha restored. Nudges: Elite-gated, session cap (5), per-page (1). FUNC-038 (1.5px border) + FUNC-039 (missing nudgeFadeIn keyframes). All depend on /assets/agents/*.svg which are 404 on prod (FUNC-036). |
...
| 74 | Prompts page | Loads? 15 prompts? Filters work? Copy works? Prefill to SitRoom works? | DONE SESSION 36 — PASS. 15 prompts across 5 categories (trade/roster/draft/waiver/dynasty). Filter buttons toggle correctly. Copy uses clipboard API + execCommand fallback. Prefill saves to localStorage, read by warroom.js. XSS-escaped. 0 JS errors. FUNC-037 (nav replaces Pricing with Prompts, orphan page) already ticketed. 404 on prod (FUNC-036). |
