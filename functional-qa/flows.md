...
| 16 | Player comparison | Compare 2 players. Stats aligned? Same season? Difference calculations correct? | RE-AUDIT SESSION 72 — FUNC-069 FILED. Ship Loop XSS rewrite (inline onclick → data-attributes + addEventListener) is structurally correct, but compare search on player profile page returns 0 results: player.js:750 reads data.players but API returns data.items. Standalone /compare/{id1}/{id2} works (0 errors, 11 stat rows). |
| 17 | Player charts | Radar/scatter/trend for a player. Data matches profile? Axes labeled correctly? | RE-AUDIT SESSION 72 — PASS. Ship Loop charts.js escapeAttr fix on player_id in option value verified. Defensive only (IDs are alphanumeric). 76 option elements, 0 JS errors. |
