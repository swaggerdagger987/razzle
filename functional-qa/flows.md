...
| 4 | Screener: Sort | Sort every stat column. #1 player is actually the leader? Reverse sort works? | RE-AUDIT SESSION 23 — CODE PASS, PROD FAIL. Sort direction fix in lab-panels.js verified correct (va-vb, was vb-va — 4 panel sort functions inverted). Null sentinel fix verified. sql_limit 500→2000 verified. PROD: derived column sorts (WOPR, target_share) return PPR order, not sorted by derived metric (FUNC-020). Panel sorts inverted on prod. All fixes in ship/launch-fixes, NOT DEPLOYED to master. SQL-sortable columns (rushing_yards, fantasy_points_ppr) sort correctly on prod. |
...
| 10 | Screener: Pin player | Pin a row. Survives sort? Survives filter? Unpin works? Multiple pins? | RE-AUDIT SESSION 23 — CODE PASS. Push pin emoji replaced with CSS-mask SVG icon (.pin-icon class). 26 pin-icon elements render on local (25 rows + 1 header). Theme-aware: var(--ink) default, var(--orange) active. No blinking. FUNC-008 pin cache intact. Fix NOT DEPLOYED to prod (prod still uses emoji). |
...
| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 13 | Player profile: NFL | Click a player name. Profile loads? Stats match screener row? Season selector works? | RE-AUDIT SESSION 23 — CODE PASS, PROD FAIL. FUNC-018 fix verified: base href="/" added to player.html. Local: /player/00-0036389 loads with 0 JS errors, "Jalen Hurts" renders. PROD: /player/{id} still broken — 3 JS errors (styles.css/app.js/player.js resolve to /player/ prefix). FUNC-019 filed. player.html?id= works on both local and prod (0 JS errors). FUNC-004 (headshots) still requires DB upload. |
...
| 16 | Player comparison | Compare 2 players. Stats aligned? Same season? Difference calculations correct? | RE-AUDIT SESSION 23 — CODE PASS, PROD FAIL. FUNC-018 fix verified: base href="/" added to compare.html. Local: /compare/00-0036389/00-0037013 loads with 0 JS errors, player data renders. PROD: /compare/{id1}/{id2} still broken — 3 JS errors (same relative path issue). compare.html?id1=X&id2=Y works on both. FUNC-019 covers deployment. |
