...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 63 — PARTIAL. FUNC-063 backend fix verified: `_grade_from_percentile()` shared function, 8-tier scale, 15 call sites, 0 old inline defs. IEEE 754 rounding in core.py clean (McCaffrey 416.6, Robinson 370.8). But FUNC-064 REGRESSION: 5 frontend renderers don't handle B+/C+ grades (efficiency.html, consistency.html, schedule.html, stocks.html, lab-panels.js SOS). B+ renders as unstyled or RED. |
...
| 62 | Dashboard / Stat Leaders | Summary stats populated? Leaders match screener data? Category switching works? | RE-AUDIT SESSION 63 — PARTIAL. FUNC-063 backend fix verified (8-tier grade scale unified across all 15 call sites in dashboards.py). FUNC-064 OPENED: frontend regression — efficiency.html, consistency.html, schedule.html, stocks.html standalone + lab-panels.js SOS panel don't handle new B+/C+ grades. All Pro-gated so free users unaffected. Core calculation accuracy confirmed (IEEE 754 rounding clean). |
