...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 31 — PARTIAL FIX, CASCADE FOUND. efficiency_rankings FIXED in code (QB PPO=pass_att+carries). BUT 3 sister functions STILL BROKEN: stock_watch(line 739), report_cards(line 1244), season_awards(line 1569) all use opportunities=targets+carries for QBs. FUNC-028 filed. Prod: "Most Efficient" award = Goff (QB). Report Card Mayfield eff=A. Career mode YPC math exact. _CORR_STATS td_rate SQL CASE correct. |
