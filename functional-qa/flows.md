...
| 4 | Screener: Sort | Sort every stat column. #1 player is actually the leader? Reverse sort works? | RE-AUDIT SESSION 24 — PROD PASS (screener). POST /api/screener/query sorts correctly for derived metrics: WOPR desc = Wilson(0.925)>JSN(0.867)>Amon-Ra(0.746). python_sort + sql_limit=2000 + null sentinels deployed. GET /api/players still falls back to PPR for derived sorts (FUNC-020 downgraded to P2, no user-facing impact). SQL-sortable columns sort correctly. |
...
| 10 | Screener: Pin player | Pin a row. Survives sort? Survives filter? Unpin works? Multiple pins? | RE-AUDIT SESSION 24 — PROD PASS. Pin icons deployed (CSS-mask SVG). Session 25 fix: expand arrow disabled on pinned rows (rowIdx null guard, lab.js:1718) prevents orphan DOM elements in pinned tbody. Code verified: `rowIdx != null` check before rendering clickable expand arrow. |
...
| 16 | Player comparison | Compare 2 players. Stats aligned? Same season? Difference calculations correct? | RE-AUDIT SESSION 24 — PROD PASS. FUNC-019 RESOLVED: /compare/00-0036389/00-0034796 loads with 0 JS errors, 20 diff bars rendered. Session 24-25 fixes deployed: diff bar Math.abs for negative stats, normalize widths to 100% (no overflow), clipboard fallback, skip-link scrollIntoView. All verified on prod. |
| 17 | Player charts | Radar/scatter/trend for a player. Data matches profile? Axes labeled correctly? | RE-AUDIT SESSION 24 — CODE PASS. Session 25 fix: radar chart min/max range normalization supports negative stats (EPA). Old code clamped to 0, new code uses minVals/maxVals from dataset with floor=0 and range guard (>=0.001). Normalize: (raw - min) / (max - min), clamped [0,1]. Charts remain position-colored, theme-aware, DPI-scaled. |
...
| 22 | Aging Curves | Chart renders? Shows realistic age-based decline? Peak age correct per position? | RE-AUDIT SESSION 24 — CODE PASS. Session 25 fix: globalAlpha restored to 1.0 after dashed player line (was stuck at 0.7 for data points and labels). lab.js:9507 sets alpha=1.0 after stroke, line 9526 sets alpha=1.0 after label. Baseline curve draws at alpha=0.9 (line 9534), restored to 1.0 (line 9542). P2: WR/TE survivorship bias remains. |
...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 25 — PARTIAL PASS. Screener derived stats exact (WR/QB YPC/YPR/YPT/CR/CompPct all verified). FAIL: FUNC-021 filed — Efficiency Rankings PPO metric broken for QBs. opportunities=targets+carries ignores pass attempts, making all QBs appear 3-6x more efficient than RBs. Top 10 "most efficient" are ALL QBs (Prescott PPO=5.92 vs McCaffrey 0.95). TD rate also wrong for QBs (60%+ because passing TDs divided by only rushing touches). |
...
| 61 | Pricing page | All plans shown? CTAs work? Correct prices? Checkout starts? | RE-AUDIT SESSION 24 — CODE PASS. Session 25 billing fix: _handle_payment_failed no longer downgrades plan (was immediately setting to free). Now marks subscription status='payment_failed' and waits for Stripe retry. Actual downgrade only on subscription.deleted. Lifetime plans guarded. Pricing page UI unchanged. |
