...
|---|------|-------------|--------|
| 1 | Landing -> Lab | CTA click, initial data load, screener populates with real player data | RE-AUDIT SESSION 28 — PASS. Ship Loop NULL age fix verified (tools.py has_ages fallback). Prod: 0 JS errors, landing page hero/CTAs/featured data loading. Dynasty risers sensible (Nacua 23.4 PPG age 25). Rookie board alphabetical (no draft picks yet, expected). FUNC-024 filed: has_ages scoping bug (NameError if dynasty_risers query throws). Dark mode clean. Lab: 25 data rows, 607 players. |
...
| 4 | Screener: Sort | Sort every stat column. #1 player is actually the leader? Reverse sort works? | RE-AUDIT SESSION 29 — PARTIAL FAIL. FUNC-025 filed: safe_sorts whitelist missing td_rate, fumble_rate, passer_rating, ay_per_att. Sorting by these columns silently falls back to PPR. yards_per_carry and catch_rate sort correctly (in whitelist). SQL-sortable columns correct. WOPR python_sort still works. |
...
| 7 | Screener: Week filter | Select Week 1. Stats are single-week, not season totals? Switch back to All Weeks. | RE-AUDIT SESSION 29 — STILL FAILING. FUNC-023 code fix correct (core.py:499 early return when week>0) but NOT deployed. Prod: Allen Week 1 PPR=38.8 (correct) but scramble=48, gl_car=17 (season totals, identical to all-weeks). Fix awaiting Render deploy. |
...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 29 — CODE FIX VERIFIED, NOT DEPLOYED. Ship Loop commit 4163512: (1) dashboards.py adds SUM(s.attempts) as pass_attempts to efficiency query, QBs use pass_attempts+carries for PPO, pass_tds/pass_attempts for td_rate. (2) core.py _enrich_with_derived_stats: QB-specific td_rate=passing_tds/attempts, fumble_rate=fumbles/(attempts+carries). (3) players.py career stats: same QB branch. (4) _CORR_STATS td_rate: position-aware SQL CASE. All code correct. Prod still broken: Prescott PPO=5.92 (should be ~0.48), Stafford td_rate=158.6% (should be 7.7%). Awaiting Render deploy. |
...
| 61 | Pricing page | All plans shown? CTAs work? Correct prices? Checkout starts? | RE-AUDIT SESSION 26 — PASS. Session 25 billing fix verified: _handle_payment_failed marks status='payment_failed' only (billing.py:537-540), downgrade exclusively in _handle_subscription_deleted (billing.py:513). Lifetime plans guarded. Pricing page UI unchanged. |
