...
| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 32 | Target Share / Air Yards | Values are percentages that sum correctly per team? WOPR calculated right? | RE-AUDIT SESSION 47 — FUNC-050 (P1). Air Yards panel has 2 field name mismatches: (1) AY% column reads `p.air_yard_pct` but API returns `air_yards_share` — shows "-%". (2) GP column reads `p.games_played` but API returns `games` — shows "-". Both column sorting also broken. WOPR/RACR/aDOT data correct. |
