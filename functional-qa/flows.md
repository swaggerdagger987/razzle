...
| 2 | Screener: Position filter | Filter QB/RB/WR/TE individually. Count matches. Remove filter. Table resets? | RE-AUDIT SESSION 46 — FUNC-047 (P0). Position filter PASS but **team filter critically broken**: 237 players have stale `players.team` (e.g., Goff=LA not DET, Barkley=NYG not PHI, McCaffrey=CAR not SF). Filter by DET shows Swift/Hockenson (left years ago), misses Goff/Montgomery (actual current DET). `INSERT OR IGNORE` in nflverse_adapter.py:445 never updates team. |
| 3 | Screener: Multi-filter | Chain 3 filters (pos + team + min stat). Results are the correct intersection? | RE-AUDIT SESSION 46 — Position+stat filters PASS. Team component BROKEN by FUNC-047. RB+DET returns Swift (left 2023) not Montgomery (actual DET RB). Multi-filter with team is unreliable. |
...
| 10 | Screener: Pin player | Pin a row. Survives sort? Survives filter? Unpin works? Multiple pins? | RE-AUDIT SESSION 45 — FUNC-045 STILL OPEN (P1). Ship Loop fixed FUNC-044 (prospect links + aging curves) but did NOT fix watchlist star onclick at lab.js:1729. escapeAttr(full_name) in inline handler still dead-clicks for 11+ apostrophe players. Pin functionality itself OK. |
...
| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 13 | Player profile: NFL | Click a player name. Profile loads? Stats match screener row? Season selector works? | RE-AUDIT SESSION 42 — PASS. Ship Loop fixes verified: (1) Breakout detection threshened (prevPPG>=8, prevGP>=10, currGP>=10) — prevents injury-recovery false positives. Lamar +183% (2019) still shows correctly. (2) Rate metrics season_type='regular' filter added to profile query (players.py:721) — prevents playoff data contamination. Prod: 0 JS errors, 13 buttons, 6 inputs. Not deployed. |
| 14 | Player profile: Career stats | Career numbers add up across seasons? Per-game averages calculated correctly? | RE-AUDIT SESSION 45 — FUNC-046 FILED (P2). Career sum totals have IEEE 754 artifacts: McCaffrey fantasy_points_std=1889.2999999999997 (should be 1889.3), Chase fantasy_points_ppr=1526.3000000000002. Fix: round(sum(...), 1) in players.py:743. Season-by-season totals correct. |
...
| 16 | Player comparison | Compare 2 players. Stats aligned? Same season? Difference calculations correct? | RE-AUDIT SESSION 39 — PROD PASS. Ship Loop fixed relative→absolute asset paths (FUNC-018), but <base href="/"> already handled resolution. Prod: /compare/00-0034796/00-0034857 renders correctly (Lamar vs Allen, radar overlay, career arc, 11 head-to-head stat rows). 0 JS errors, 12 buttons. No regressions. |
...
| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 32 | Target Share / Air Yards | Values are percentages that sum correctly per team? WOPR calculated right? | RE-AUDIT SESSION 42 — PASS. Ship Loop season_type='regular' filter added to air yards rate metrics (analytics.py:2164) and stat explorer (analytics.py:1175). All 7 player_week_metrics queries now filtered. Prod data sensible: buy low (Thornton aDOT=16.6, RACR=0.27), sell high (Rice aDOT=4.3, RACR=1.69). Not deployed. |
...
| 35 | Opportunity Share | Opportunity = targets + carries? Per-team shares sum to ~100%? | RE-AUDIT SESSION 46 — FUNC-047 impacts: McCaffrey team=CAR (stale, should be SF). Team-level share calculations use `p.team` which is wrong for 237 players. Target distribution endpoint for DET includes Swift's stats from his actual 2025 team, corrupting DET carry shares. Per-team sums can't be trusted until FUNC-047 fixed. |
...
| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 57 | Sidebar navigation | Every sidebar item loads its panel? No dead links? Category headers correct? | RE-AUDIT SESSION 42 — PASS. Ship Loop XSS fix verified: escapeHtml→escapeAttr in onclick attributes for recent/favorite panel chips (lab.html:4684,4738). escapeAttr encodes single quotes (&#39;) preventing breakout from onclick='switchPanel(...)' context. Prod: 0 JS errors, 69 sidebar items. Not deployed. |
