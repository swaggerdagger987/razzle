...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 38 — CODE FIX PASS, PROD STILL BROKEN. Code: QBs use pass_attempts+carries for opp, pass_tds/pass_attempts for td_rate (dashboards.py:121-122). Prod: Dak opp=53 (carries only), td_rate=60.4% (totalTD/carries). Ship Loop deleted FUNC-035 but fix awaits deployment. |
