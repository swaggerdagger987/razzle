...
| 16 | Player comparison | Compare 2 players. Stats aligned? Same season? Difference calculations correct? | RE-AUDIT SESSION 49 — FUNC-053 (P1). Lab sidebar "Compare" in START HERE section is broken: `data-panel="compare"` has no panel definition (only "career-compare" exists) AND "compare" missing from FREE_PANELS → shows Pro gate for free users, blank panel for Pro users. Standalone /compare/{id1}/{id2} still works (session 39 verified). |
...
| 37 | Regression candidates | TD regression logic sound? Flagging high-TD players with low expected TDs? | RE-AUDIT SESSION 50 — FUNC-052 STILL BROKEN (lab-panels.js). NEW FUNC-055: standalone regression.html ALSO broken — Ship Loop Session 32 added null guards (||[]) before .length but field names still wrong (regression_up/regression_down vs API's positive_regression/negative_regression, pos_avg_rates vs pos_avg_td_rates, regression_delta vs td_diff). Null guards mask crash but page always shows empty state. NEW FUNC-056: Lab pace tracker panel uses p.projections/s.milestone vs API's p.milestones/s.target — milestone progress bars always empty. |
...
| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 57 | Sidebar navigation | Every sidebar item loads its panel? No dead links? Category headers correct? | RE-AUDIT SESSION 49 — FUNC-053 (P1) + FUNC-054 (P2). 3 sidebar items have NO panel definition in lab-panels.js: (1) "Compare" in START HERE (data-panel="compare") — only "career-compare" exists, also missing from FREE_PANELS so shows Pro gate. (2) "Mock Draft" (data-panel="mockdraft") — Pro-locked, blank for Pro users. (3) "Athletic Radar" (data-panel="proradar") — Pro-locked, blank for Pro users. 72 sidebar items total, 67 panel definitions, 3 orphans + screener (handled separately) + JS false positive. |
