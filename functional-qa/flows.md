...
| 4 | Screener: Sort | Sort every stat column. #1 player is actually the leader? Reverse sort works? | RE-AUDIT SESSION 39 — CODE FIX VERIFIED. Ship Loop overhauled derived stat sorting: over-fetch 500 rows by PPR, enrich, Python re-sort by actual derived key, slice. Prod still broken (target_share sort returns PPR order: Nacua 31.1% #1 but JSN 35.7% should be). Code fix correct. P2 FUNC-042: over-fetch cap too low for career mode (WR career 728 > 500 cap). Screener POST uses 2000 cap (adequate for single-position). |
...
| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 13 | Player profile: NFL | Click a player name. Profile loads? Stats match screener row? Season selector works? | RE-AUDIT SESSION 39 — PROD PASS. Ship Loop fixed relative→absolute asset paths (FUNC-018), but <base href="/"> already handled resolution. Prod: /player/00-0036389 renders full styling (warm sand, chunky borders, radar chart, career arc, season log, combine data). 0 JS errors, 13 buttons, 6 inputs. Jalen Hurts profile correct. No regressions. |
...
| 19 | Trade Values | Values load? Positional adjustment? Do elite young WRs > aging vets? Sensible tiers? | RE-AUDIT SESSION 35 — PASS. Prod: Nacua(WR,24.8)=95.8, McBride(TE,26.3)=95.6, Bijan(RB,24.1)=95.4, Gibbs(RB,24.0)=95.3. P2 FUNC-003 clustering remains (top 25 all 90.7-95.8, all tier=1). P2: TE overvaluation (5 TEs in top 25 due to position-relative production scores). P2: low GP players (Kraft 8GP, Rice 8GP) ranked highly. |
...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 39 — CODE FIX PASS, PROD STILL BROKEN. Prod confirmed: Dak opp=53 (carries only, should be pass_attempts+carries), Allen opp=112 (carries only, should be ~607). Code fix verified in dashboards.py:121-122. Fix awaits deployment. |
...
| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 63 | Bureau: League Intel | Sleeper connect flow? Roster loads? Insights generated from real league data? | RE-AUDIT SESSION 37 — PASS with P2. Connect card clean, 0 JS errors. Demo league functional but says "12-team" with only 10 managers (FUNC-041). Bureau APIs verified: roster-depth-lookup POST works (McCaffrey 24.51 PPG), Monte Carlo works (Lamar mean=25.32, stdev=6.19). Prod headshots all empty (part of FUNC-036 DB rebuild). |
