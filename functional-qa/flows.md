...
| 37 | Regression candidates | TD regression logic sound? Flagging high-TD players with low expected TDs? | RE-AUDIT SESSION 49 — FUNC-052 STILL BROKEN. Ship Loop d697403 fixed FUNC-049/050/051 but did NOT fix FUNC-052. Lines 5213-5215 still use `data.buy_low`/`data.sell_high`/`data.position_rates` (should be `positive_regression`/`negative_regression`/`pos_avg_td_rates`). Line 5222/5259 still use `p.diff` (should be `p.td_diff`). Ticket remains open. |
