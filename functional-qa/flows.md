...
| 7 | Screener: Week filter | Select Week 1. Stats are single-week, not season totals? Switch back to All Weeks. | RE-AUDIT SESSION 33 — STILL FAILING ON PROD. Allen Week 1 vs All Weeks: gl_carries=17 (IDENTICAL), gl_tds=11 (IDENTICAL), scramble_attempts=48 (IDENTICAL), scramble_yards=426 (IDENTICAL). Core stats correct (passing_yards 394 vs 3668). PBP-derived stats leak season totals into weekly view. FUNC-023 code fix exists but NOT deployed. |
...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 33 — STILL BROKEN ON PROD. ALL FOUR efficiency functions affected (not just cascade). /api/efficiency-rankings: Prescott PPO=5.92 Opp=53 (should be ~567 with 480+ pass_attempts). /api/season-awards: Goff still wins "Most Efficient". /api/report-cards: Mayfield eff=A. Code fix exists locally but NEVER DEPLOYED to Render. FUNC-028 ticket updated with fresh evidence. Blocking: push ship/launch-fixes + Render deploy. |
...
| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 63 | Bureau: League Intel | Sleeper connect flow? Roster loads? Insights generated from real league data? | RE-AUDIT SESSION 33 — PASS. Demo mode works: "See a demo league" → sample league with 10 managers, championship odds, warning banner. 0 JS errors. All 7 Bureau panels (Roster Depth, Build Profiles, Trade Network, Waiver Tendencies, Power Rankings, H2H Rivalry, Self-Scout) code-audited: no critical bugs, all division guards present, XSS safe, null-guarded. All 3 Bureau API endpoints exist on prod (roster-depth-lookup, monte-carlo/projections, league-trade-finder). Monte Carlo returns correct data (CMC mean=24.51 PPG). |
| 64 | Situation Room | Canvas loads? Agents rendered? Interaction works? | RE-AUDIT SESSION 33 — PASS. 0 JS errors on prod. 6 agents with personality quotes, expertise tags, "Ask [Name]" buttons. 62 buttons, 16 inputs. First-run setup guide, demo briefings, "What can I ask?" panel all functional. |
