...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 55 — Local PASS: round() fix works (0 IEEE 754 artifacts in top 10 RBs). Prod FUNC-059 still present (McCaffrey 416.59999...). FUNC-058 (snap sync, local only) and FUNC-042 (GET sort, prod) unchanged. |
...
| 43 | VORP | Value over replacement calculated? Replacement level defined per position? Sensible? | RE-AUDIT SESSION 55 — PASS + FUNC-060. VORP panel fine. Stock Watch adjacent: PPO null for 20/21 TEs (>50 opp threshold too high). Efficiency grade always "C" fallback for these TEs. Report-cards uses different logic — TE efficiency grades correct there. FUNC-060 ticket filed. |
