...
| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 13 | Player profile: NFL | Click a player name. Profile loads? Stats match screener row? Season selector works? | RE-AUDIT SESSION 53 — FUNC-057 FIXED. Ship Loop 4ed32ce added Array.isArray(d) guard in career.html:965 + percentiles.html:608. Both standalone pages redirect to Lab panel anyway (line 7: window.location.replace). Lab career panel uses its own search (buildPlayerSearch). Fix is defensive for iframe edge case. Prod verified: player/00-0033873 loads Mahomes correctly. |
...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 56 — FUNC-059 FIXED locally. ROUND(SUM,1) in players.py career screener + ROUND(SUM,2) in server.py season_stats + ROUND(SUM,1) in tools.py success_rate/game_script. McCaffrey career 2515.3, season 416.6 — 0 artifacts in top 10. player_season_stats table still has artifacts (table not rebuilt) but API endpoints compute derived metrics (PPG) that naturally clean them. Not user-facing. |
...
| 41 | Consistency | Week-to-week consistency calculated? Boom/bust rates make sense? | RE-AUDIT SESSION 57 — PASS. Ship Loop import refactor (math+defaultdict hoisted). Endpoint 200, Maye CoV=0.26 StdDev=5.39 Grade=A+. College consistency also works (math.sqrt rename from _math). 0 JS errors on panel. |
...
| 43 | VORP | Value over replacement calculated? Replacement level defined per position? Sensible? | RE-AUDIT SESSION 57 — PASS. Ship Loop import refactor (math+defaultdict hoisted in dashboards.py). VORP+Stock Watch endpoints 200. FUNC-060 still open (14/19 rising TEs null PPO). 0 JS errors on panel. |
...
| 62 | Dashboard / Stat Leaders | Summary stats populated? Leaders match screener data? Category switching works? | RE-AUDIT SESSION 57 — FUNC-061/062 STILL OPEN. Ship Loop import refactor (math+defaultdict hoisted) didn't change award logic. Chism III still wins Most Efficient (3 opps), Bo Nix still Volume King (all QBs). Red Zone King 0 GL TDs locally (PBP=0 rows) but prod has real data (McCaffrey 12 GL TDs). 0 JS errors on awards panel. |

## Group 9: Bureau & Situation Room (the paid tier)

| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 63 | Bureau: League Intel | Sleeper connect flow? Roster loads? Insights generated from real league data? | RE-AUDIT SESSION 52 — Phase C Bureau hardening PASS. Sleeper connect: 10s timeout, differentiated errors (AbortError vs not-found vs network), Enter key support, loading state on button. League data: resp.ok checks, empty roster guard, Array.isArray on users, 15s timeout, retry support. Manager profiles: multi-season crawl, 12 behavioral traits, XSS escaped. Trade finder: Pro-gated, 25% value gap, 15s timeout, retry on error. Pressure map: composite 0-100 scoring (record+panic+trade+FAAB), top 3 free/rest blurred with pointer-events:none. Monte Carlo: Web Worker + inline fallback, Box-Muller transform, 10K sims, scenario explorer (injury toggle + trade what-if), re-simulation on scenario changes. Bridge CTAs: 7 prefillScenario calls with escapeJsString. 93 escape function calls total. Backend: roster-depth-lookup returns correct PPG (McCaffrey 24.51, Chase 19.6), monte-carlo/projections returns valid distributions. Demo league: 12 managers, sensible odds. 0 JS errors, 0 content leaks, 0 XSS vectors. |
