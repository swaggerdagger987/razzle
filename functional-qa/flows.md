...
| 16 | Player comparison | Compare 2 players. Stats aligned? Same season? Difference calculations correct? | RE-AUDIT SESSION 74 — FUNC-069 STILL OPEN. player.js:750 still reads data.players (API returns data.items). Ship Loop has not touched player.js since session 72. Standalone /compare works. Profile compare search remains broken. |
...
| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 63 | Bureau: League Intel | Sleeper connect flow? Roster loads? Insights generated from real league data? | RE-AUDIT SESSION 74 — PASS. FUNC-068 FIXED by Ship Loop (935a6f7). _hrAc clearTimeout in both .then() and .catch(). _prAc/_wvAc clearTimeout on success path (catch path missing but self-cleaning 10s timers, zero practical impact). 0 JS errors. |
