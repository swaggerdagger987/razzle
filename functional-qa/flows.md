...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 66 — FUNC-066 FILED. Grade scale inconsistency: core.py uses 12-tier (A-/B-/C-/D+), dashboards.py uses 8-tier (A+/A/B+/B/C+/C/D/F). McCaffrey eff=C (report cards) vs eff=D (efficiency rankings). Buy-sell produces A-/C-/D+/D- grades. Frontend handles both via charAt(0) but displayed text inconsistent across pages. 33/34 API endpoints return 200 on prod. Half-PPR verified (MCF 416.6 PPR, 365.6 HPPR). |
...
| 62 | Dashboard / Stat Leaders | Summary stats populated? Leaders match screener data? Category switching works? | RE-AUDIT SESSION 67 — PASS with P2. FUNC-067 FILED: Awards runner-up PPG falsy-zero (lab-panels.js:6894, `r.ppg || ''` shows empty for PPG=0). Winner rendering correct (line 6858). Low practical impact — runners-up unlikely to have PPG=0. Ship Loop falsy-zero sweep missed this one. |

## Group 9: Bureau & Situation Room (the paid tier)

| # | Flow | What to Test | Status |
|---|------|-------------|--------|
| 63 | Bureau: League Intel | Sleeper connect flow? Roster loads? Insights generated from real league data? | RE-AUDIT SESSION 66 — PASS. Ship Loop monte-carlo-worker.js null guard on rp.wins/rp.losses verified (line 50-51). Bureau page 0 JS errors, 55 links, 14 buttons, 8 inputs. escapeJS hex escape hardening (app.js) verified — 14 call sites in league-intel.html all safe. |
