...
| 19 | Trade Values | Values load? Positional adjustment? Do elite young WRs > aging vets? Sensible tiers? | RE-AUDIT SESSION 34 — PASS. Prod: Chase(WR,26)=95.8, Gibbs(RB,24)=95.0, Bijan(RB,24)=94.7, Bowers(TE,23)=94.0. Top 20 dynasty-sensible with age/production/scarcity weighting. P2 FUNC-003 clustering remains (top 20 all 91.5-95.8, all tier=Elite). |
...
| 36 | Efficiency metrics | YPC, YPR, YPT calculated correctly from raw stats? Not showing NaN/Infinity for 0 attempts? | RE-AUDIT SESSION 34 — CODE FIX VERIFIED, NOT DEPLOYED. Commit 4163512: SUM(s.attempts) added to efficiency query, QB opp=pass_attempts+carries, QB td_rate=passing_tds/attempts. Direct DB: Mayfield pass_att=570, carries=60, opp=630. API still returns Opp=60 (server running old code). Prod identical. FUNC-030 filed: restart server + deploy to Render. |
