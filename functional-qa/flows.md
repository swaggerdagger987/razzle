...
| 10 | Screener: Pin player | Pin a row. Survives sort? Survives filter? Unpin works? Multiple pins? | RE-AUDIT SESSION 44 — FUNC-045 FILED (P1). Watchlist star onclick uses escapeAttr(full_name) in inline handler — dead click for 11+ apostrophe players (Ja'Marr Chase, D'Andre Swift, etc.). Same root cause as FUNC-044. Pin functionality itself OK. Ship Loop localStorage persistence for _pinnedDataCache verified correct. |
...
| 22 | Aging Curves | Chart renders? Shows realistic age-based decline? Peak age correct per position? | RE-AUDIT SESSION 43 — FUNC-044 FILED (P1). escapeAttr-in-onclick dead clicks for 11 players with apostrophes (Ja'Marr Chase, De'Von Achane, D'Andre Swift, etc.). Ship Loop c613fc6 fixed same pattern in roster search but missed aging curves (lab.js:9687) and prospect profiles (lab.js:1765). Chart rendering still fine. P2: WR/TE survivorship bias remains. |
...
| 29 | Breakouts | Candidates shown with data? Breakout criteria visible and reasonable? | RE-AUDIT SESSION 42 — PASS. Ship Loop season_type='regular' filter on target_share query (analytics.py:705) verified correct. Breakout data sensible (Gordon 23.6, Jeanty 21.4). NEW: FUNC-043 filed — 22/50 rbs_score values have IEEE 754 artifacts (e.g., 21.799999999999997). P2 fix: round(max(0, gap + age_bonus), 1). |
...
| 35 | Opportunity Share | Opportunity = targets + carries? Per-team shares sum to ~100%? | RE-AUDIT SESSION 37 — PASS. Prod 2025: McCaffrey opp%=44.0 (tgt/g=7.6, car/g=18.3). Dominators: 30 total (26 RB + 4 WR). Taylor dom=79.1, Adams dom=46.9, JSN dom=42.1. FUNC-005 confirmed fixed. |
...
| 64 | Situation Room | Canvas loads? Agents rendered? Interaction works? | RE-AUDIT SESSION 44 — PASS. Ship Loop warroom.js fixes verified: (1) filter format resilience (f.key || f.stat || '?') — correct fallback chain. (2) LLM timeout fix — clearTimeout moved after resp.json() so timeout covers full response lifecycle. Both callServerLLM and callFreeLLM fixed consistently. No regressions. |
