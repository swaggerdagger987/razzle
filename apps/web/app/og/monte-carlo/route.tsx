...
export async function GET(req: Request) {
  ...
  const fromSnapshot = snapshot?.odds?.length ? snapshot.odds : null;
  const live = fromSnapshot ? null : await fetchMonteCarlo(league, user);
  const isDemo = !fromSnapshot?.length && !live?.length;
  const odds = fromSnapshot ?? (isDemo ? DEMO_ODDS : live!);
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", fontSize: 20, color: "#5c4a3d", marginBottom: 20 }}>
          {`playoff + title odds from roster sims${fromPanel ? " · from your sim board" : isDemo ? " · sample preview" : ""}`}
        </div>
