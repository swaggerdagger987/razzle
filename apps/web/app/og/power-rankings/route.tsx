...
const DEMO_ROWS: PowerRow[] = [
  { rank: 1, roster_id: 1, team: "Dynasty Dukes", record: "9-4", ppg: 128.4, opp_ppg: 112.1, differential: 16.3, luck: 1.2 },
  { rank: 2, roster_id: 2, team: "Win-Now LLC", record: "8-5", ppg: 121.2, opp_ppg: 115.8, differential: 5.4, luck: -0.5 },
  { rank: 3, roster_id: 3, team: "Rebuild FC", record: "5-8", ppg: 108.6, opp_ppg: 119.2, differential: -10.6, luck: 0.8 },
  { rank: 4, roster_id: 4, team: "Comfort Crew", record: "7-6", ppg: 114.3, opp_ppg: 113.9, differential: 0.4, luck: -1.1 },
  { rank: 5, roster_id: 5, team: "Late Round Larry", record: "6-7", ppg: 105.1, opp_ppg: 118.4, differential: -13.3, luck: -0.3 },
];
    ...
    (
      ...
      >
        ...
        <div style={{ display: "flex", fontSize: 20, color: "#5c4a3d", marginBottom: 14 }}>
          {`beyond W-L — points differential & luck index${isDemo ? " · sample preview" : ""}`}
        </div>

        {leader ? (
          <div style={{ display: "flex", fontFamily: "Caveat", fontSize: 30, color: "#d97757", marginBottom: 12 }}>
          ...
          {rows.map((row) => {
            ...
            return (
              <div key={row.roster_id} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <div style={{ display: "flex", width: 36, fontSize: 16, fontWeight: 700, color: "#8a7565" }}>
                  #{row.rank}
                </div>
                <div style={{ display: "flex", width: 120, flexDirection: "column" }}>
                  <div style={{ display: "flex", fontSize: 17, fontWeight: 700 }}>{teamLabel(row.team)}</div>
                  <div style={{ display: "flex", fontSize: 13, color: "#8a7565" }}>{row.record}</div>
                </div>
                <div
                  style={{
                    display: "flex",
                ...
                <div style={{ display: "flex", width: 52, fontSize: 16, fontWeight: 700, color }}>
                  {row.differential > 0 ? "+" : ""}
                  {row.differential}
                </div>
                <div style={{ display: "flex", fontSize: 13, color: "#8a7565", width: 72 }}>
