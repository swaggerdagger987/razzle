...
export async function GET(req: Request) {
  ...
  const live = forceDemo ? null : await fetchPressureMap(req, league);
  const isLive = Boolean(live?.rows?.length);
  const isDemo = forceDemo || !isLive;
  ...
  return new ImageResponse(
    (
      ...
      >
        ...
        {/* Always-on watermark band — matches Self-Scout + Trade Finder OG (T6 screenshot gravity) */}
        <div
          style={{
            display: "flex",
