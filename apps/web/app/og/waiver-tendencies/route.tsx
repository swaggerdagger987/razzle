...
function hawkeyeWaiverRoomQuestion(hero: WaiverRow): string {
  return `What waiver move should I expect from ${hero.team} this week? (${hero.archetype.split(" (")[0] ?? hero.archetype})`;
}

async function fetchWaiverTendencies(
  req: Request,
  leagueId: string,
): Promise<WaiverData | null> {
...
export async function GET(req: Request) {
  ...
  const live = await fetchWaiverTendencies(req, league);
  ...
  const hawkeyeRoomPath = toRoom({
    agentId: "hawkeye",
    question: hawkeyeWaiverRoomQuestion(hero),
