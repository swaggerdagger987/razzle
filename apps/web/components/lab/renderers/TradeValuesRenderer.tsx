...
export function TradeValuesRenderer({ panel }: Props) {
  ...
  const q = useQuery({
    ...
    queryFn: async () => {
      ...
      return panelApiGet<ChartData>(`/api/panels/${panel.slug}${qs}`);
    },
  });
  ...
  const ogSnapshotRows = useMemo((): OgSnapshotRow[] => {
    return players.slice(0, 6).map((p, i) => {
      const rank = formula ? i + 1 : (p.rank ?? i + 1);
      const stat = formula ? (p.formula_score ?? 0) : (p.trade_value ?? 0);
      const statLabel = formula ? `${rank} · ${formula.name}` : `${rank} · Value`;
      return {
        name: p.full_name,
        position: p.position,
        team: p.team,
