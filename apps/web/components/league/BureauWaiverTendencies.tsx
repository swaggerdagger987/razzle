        ...
        <section className="chunky bg-bg-card p-4">
          ...
          {topAdder && (
            <p className="mt-1 font-bold" style={{ fontFamily: "var(--font-display)" }}>
              {topAdder.team} · {topAdder.adds} adds · {topAdder.archetype}
            </p>
          )}
          {hoarder && hoarder.roster_id !== topAdder?.roster_id && (
            <p className="text-sm text-ink-medium" style={{ fontFamily: "var(--font-hand)" }}>
              FAAB watch: {hoarder.team} spent ${hoarder.faab_spent} on {hoarder.claim_attempts} claims
            </p>
          )}
          <Link
            href={
              toRoom({
                agentId: "hawkeye",
                question: `Who should I block on waivers in this league? ${topAdder?.team ?? "Top adder"} has ${topAdder?.adds ?? 0} adds.`,
        ...
        {rows.map((row, i) => (
          ...
          >
            <div className="mb-2 flex items-start justify-between gap-2">
              ...
              >
                {row.archetype.replace("The ", "").split(" ")[0]}
              </span>
            </div>
            <p className="text-sm text-ink-medium" style={{ fontFamily: "var(--font-hand)" }}>
              {row.archetype} · {row.claim_attempts} claim attempts
            </p>
          </div>
        ))}
      </section>
