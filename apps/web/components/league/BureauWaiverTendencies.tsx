    ...
    <div className="flex flex-col gap-6">
      <header className="chunky bg-bg-card p-6">
        ...
        <p className="text-ink-medium mt-1 text-sm" style={{ fontFamily: "var(--font-mono)" }}>
          {rows.length} teams ranked by waiver adds this season
        </p>
      </header>
      ...
      {leader && !hasError && (
        <section className="chunky bg-bg-card p-4">
          ...
          <p className="mt-1 text-sm" style={{ fontFamily: "var(--font-hand)" }}>
            {leader.team} — {leader.adds} adds, ${leader.faab_spent} FAAB — {leader.archetype}
          </p>
          <Link
            href={
              toRoom({
                agentId: "hawkeye",
                question: `${leader.team} is "${leader.archetype}" on waivers — who should I stash before they claim?`,
                panelSlug: "waiver-tendencies",
              }) as Route
            }
            className="mt-3 inline-block text-sm text-orange underline"
          >
            ask Hawkeye about {leader.team} →
          </Link>
        </section>
      )}
      ...
      {leagueId && (
        <footer className="flex flex-wrap gap-3 text-sm">
          <Link href={`/league/${leagueId}/build-profiles` as Route} className="text-orange underline">
            ← roster build profiles
          </Link>
          <Link
            href={
              toRoom({
                agentId: "hawkeye",
                question: "Who in my league will overpay on waivers this week?",
              }) as Route
            }
