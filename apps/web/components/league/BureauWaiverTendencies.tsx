      ...
      {hero && (
        <section className="chunky bg-bg-card p-4">
          ...
          <p className="text-sm text-ink-medium" style={{ fontFamily: "var(--font-mono)" }}>
            {hero.adds} adds · {hero.drops} drops · ${hero.faab_spent} FAAB spent
          </p>
          <Link
            href={
              toRoom({
                agentId: "hawkeye",
                question: `${hero.team} is "${hero.archetype}" on waivers — who should I stash before they notice?`,
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        {rows.map((row, i) => (
          ...
          >
            ...
            <p className="text-xs text-ink-medium" style={{ fontFamily: "var(--font-hand)" }}>
              {row.claim_attempts} claim attempts this season
            </p>
          </div>
        ))}
      </section>
