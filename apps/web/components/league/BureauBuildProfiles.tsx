    ...
    <div className="flex flex-col gap-6">
      <header className="chunky bg-bg-card p-6">
        ...
        <p className="text-ink-medium mt-1 text-sm" style={{ fontFamily: "var(--font-mono)" }}>
          {rows.length} teams classified · depth-chart shape, not transaction tape
        </p>
      </header>

      {Object.keys(archetypeCounts).length > 0 && (
        <section className="flex flex-wrap gap-2">
          {Object.entries(archetypeCounts).map(([tag, count]) => (
            <span
              key={tag}
              className="text-[10px] font-bold uppercase"
              style={{
                fontFamily: "var(--font-mono)",
                color: ARCHETYPE_COLORS[tag] ?? "var(--ink)",
      ...
      {hero && (
        <section className="chunky bg-bg-card p-4">
          ...
          <p className="mt-1 text-sm" style={{ fontFamily: "var(--font-hand)" }}>
            {hero.team} ({hero.record}) — {hero.archetype}. {hero.reasoning}
          </p>
          <Link
            href={
              toRoom({
