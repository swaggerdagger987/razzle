...
def test_pro_gate_from_panel_error_wired_in_tradevalues_and_efficiency():
    for name in ("TradeValuesRenderer.tsx", "EfficiencyRenderer.tsx"):
        renderer = (
            _repo_root() / "apps/web/components/lab/renderers" / name
        ).read_text(encoding="utf-8")
        assert "ProGateFromPanelError" in renderer, name
        assert "ProUpgradeGate" not in renderer, name
