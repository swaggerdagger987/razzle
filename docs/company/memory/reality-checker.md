...
2026-05-30 | Lab OG demo fallback + H2H render fix | curl PNG via next start (6 routes 200, 58-63KB) + build + pytest + screenshot | Lab L5 / FACTORY-DOD C2-C3 verified | PASS | false-Gate-C-PASS class: PR #12 H2H built but crashed at render (HTTP 000) | keep | always run next start + curl on next/og slices, build-green is insufficient
